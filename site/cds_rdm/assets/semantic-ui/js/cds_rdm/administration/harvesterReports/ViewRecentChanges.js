...
const getRevisionErrorMessage = (error) => {
  ...
  if (status === 403) {
    return i18next.t(
      "You do not have permission to view revisions for this record."
    );
  }

  if (Array.isArray(apiErrors) && apiErrors.length > 0) {
    return apiErrors.map((item) => item.message || item.title).filter(Boolean).join(" ");
  }
  ...
  return i18next.t("An unexpected error occurred while loading revisions.");
};
...
export class ViewRecentChanges extends Component {
  ...
  async fetchPreviousRevision() {
    ...
    try {
      ...
      this.setState({
        error: getRevisionErrorMessage(error),
        loading: false,
      });
      console.error(error);
    }
